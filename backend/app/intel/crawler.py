...
def crawl_site(base_url: str, max_pages: int = 8) -> list[PageData]:
    """Fetch home + common pages. Never raises — returns what succeeded."""
    import httpx

    base = validate_url(base_url)
    parsed = urlparse(base)
    origin = f"{parsed.scheme}://{parsed.netloc}"
    urls = [base] + [origin + p for p in COMMON_PATHS[1:]]
    seen: set[str] = set()
    out: list[PageData] = []
    try:
        client = httpx.Client(timeout=FETCH_TIMEOUT, follow_redirects=True,
    ...
    try:
        for u in urls:
            if len(out) >= max_pages or u in seen:
                continue
            seen.add(u)
            try:
